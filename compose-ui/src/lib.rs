...
use compose_core::{location_key, Composer, MemoryApplier};
...
pub use primitives::{Button, ButtonNode, Column, ColumnNode, Row, RowNode, Spacer, SpacerNode, Text, TextNode};
...
pub fn run_test_composition(mut build: impl FnMut(&mut Composer<'_>)) -> TestComposition {
    let mut composition = Composition::new(MemoryApplier::new());
    composition.render(location_key(file!(), line!(), column!()), |composer| build(composer));
    composition
}
