            ...
            RowSpec::default(),
            {
                move || {
                    if progress_value > 0.0 {
                        ...
                        Row(
                            Modifier::width(200.0 * progress_value).then(Modifier::height(12.0)),
                            RowSpec::default(),
                            || {},
                        );
                    }
...
fn summary_tab() {
    Column(Modifier::padding(8.0), ColumnSpec::default(), move || {
        Text("Summary Tab", Modifier::padding(2.0));
    });
}
...
fn alternating_recursive_node(depth: usize, horizontal: bool, index: usize) {
    ...
    Column(Modifier::padding(6.0), ColumnSpec::default(), move || {
        Text(label.clone(), Modifier::padding(2.0));
        if depth > 1 {
            if horizontal {
...
fn simple_counter_placeholder() {
    Column(Modifier::padding(4.0), ColumnSpec::default(), move || {
        Text("Counter placeholder", Modifier::padding(2.0));
    });
}
